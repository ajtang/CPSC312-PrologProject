...
insert_edge(E) :-
	min_pickup(E, nodes, R),
  location(E, A, B),
  location(R, C, D),
  distance(A, B, C, D, Z),
  assert(edge(E, R, Z)),
  assert(edge(R, E, Z)).
number_list([3.5,6.7,2.2,0.8]).
...
pathfind(P,D) :-
  location(P, A, B),
  closest(location(P, A, B), driver(N,X,Loc)),
  driver_path(Loc, P),
  customer_path(P, D),
...
:-dynamic
	rpath/2.      % I need this for it to work?  Google said so.

 
% path(From,To,Dist) :- dist1(To,From,Dist).
% path(From,To,Dist) :- dist1(From,To,Dist).
